...
public class Solution {
    ...
    public Solution(List<Client> allClients, List<Vehicle> allVehicles, GridGraph graph) {
    ...
    private Vehicle findBestVehicle(Client client) {
        ...
        while(count != allVehicles.size()) {
            ...
            if (vehicle.time <= client.getVisitingTimeInterval().getStartTime()) {
                int x=vehicle.getDepot().getDepotLocation().x;
                int y=vehicle.getDepot().getDepotLocation().y;
                int[] shortestPaths=graph.dijkstra(x,y);
                int destinationNode = client.getClientLocation().x* graph.getCols()+client.getClientLocation().y;
                int shortestPath=shortestPaths[destinationNode];
                vehicle.minutes += shortestPath;
