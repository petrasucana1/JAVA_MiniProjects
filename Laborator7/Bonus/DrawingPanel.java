...
public class DrawingPanel extends StackPane {
   ...
   // private List<Stone> stones= new ArrayList<>();
    ...
    public void generateAIturn() {
        int result = 0;
        for (javafx.scene.Node node : getChildren()) {
            if (node instanceof org.example.game.Node) {
                org.example.game.Node customNode = (org.example.game.Node) node;
                setCurrentNode(customNode);
                if (hasConnectedNode()) {
                    customNode.changeColor();
                    customNode.setClicked(true);
                    setPreviousNode(customNode);
                    result = 1;
                    break;
                }
            }
